...
public class DistanceTools {
	...
	public static int[][] sortIndex(double[][] matDistance)
	{
		...
		int[][] resultats = new int[matDistance.length][matDistance.length]; // Matrice rÃƒÂ©sultat

		for( int i = 0 ; i < matDistance.length ; i++) // aller de la ligne 0 ÃƒÂ  N-1
		{
			double ligneI[] = extractRow(matDistance, i); // extraire de la matrice des distances la ligne i 
			for( int j = 0 ; j < matDistance.length; j++ ) // aller de la colonne 0 ÃƒÂ  N-1
			{
			...
			for( int l = 0; l < matDistance.length; l++ ) // puis ÃƒÂ©crire les index dans l'ordre dans rÃƒÂ©sultats
			{
