...
public class Recuit extends JFrame
{
	...
	public static Routage2 solution(int[][] matIndex, Graphe g,ArrayList<Integer> liste,PrintWriter sortie) throws IOException, InterruptedException
	{
		// On définit une route aléatoire en premier lieu
		Routage2 solutionCourante = new Routage2(g,matIndex);
		
		System.out.println("taille solution Greedy = " + solutionCourante.getDistance());
		int nbit=0;
		...
		while (temperatureRecuit > 1) {
			...
			System.out.println(meilleureRoute.getDistance() +" temperature : "+temperatureRecuit);
			//System.out.println("route = "+ meilleureRoute);
		}
		printProba.close();
