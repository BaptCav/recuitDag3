...
public class Mutation {
	...
	public static void twoOptMove(Routage routage, int i , int j) {
		//On itère pour effectuer tous les échanges du twoOptMove
		while (i!=j && routage.getNextIndex(i)!=j ) {
			swap(routage,i,j);
			i=routage.getNextIndex(i);
			j=routage.getPreviousIndex(j);
	...
	public static int[] order(int i, int j, int k){
		int[] tab = new int[3];
		if (i<j && i<k){
			tab[0]=i;
			tab[1]=Math.min(j,k);
	...
	public static void threeOptMove(Routage routage){
		int n = routage.tailleRoute();
		int i = 0;
		int j = 0;
		int k = 0;
	...
	public void changePlace(Routage routage) {
		int n = routage.tailleRoute();
		int randIndex1=0;
		int randIndex2=0;
		while (randIndex1==randIndex2){
	...
	public static void moveReverse(Routage routage) {
	...
	// Création du Pruning Neighbourghood le but de cette méthode est d'avoir les voisins les plus proches pour effectuer un 2opt move
	...
	///Amodifierquandon aura fait le mutation.calcul préalablement nécessaire pour ne pas bouffer de la ressource
