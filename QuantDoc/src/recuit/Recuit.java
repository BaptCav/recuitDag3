...
public class Recuit
{
	...
	{	
		/** 
		 * Function in charge of calculating the probability of accepting the mutated particule
		 * 
		 * @param deltaE
		 * 		Difference d'energie globale
		 * 
		 * @param deltaEpot
		 * 		Difference d'energie potentielle
		 * 
		 * @return  Returns a double which is the probability of accepting the new route.
		 *			If one of the energetic difference is negative P=1, else it's Metropolis' P.
		 */

		if(deltaEpot<=0 || deltaE<0){
			return 1;
		}
	...
	public ArrayList<Etat> ecarteEtats(Graphe g, int nombreEtat, int seed){
		/**
		 * Fonction en charge d'ecarter au maximum les spins d'un problème, a son commencement
		 * @param g
		 * 	graphe utilisé pour les routages
		 * @param nombreEtat
		 * 	nombres d'etat
		 * @param seed
		 * 	seed utilisée pour la génération de nombres aléatoires
		 * @return Une liste d'etats ou les etats ont des spins éloignés 
		 */
